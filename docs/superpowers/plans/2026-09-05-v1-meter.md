...
- Toolchain: `swift-tools-version: 6.0`, Swift 6 language mode, deployment target `macOS 14`. Build with `swift build`; run tests only through `make test` (or `make test FILTER=<SuiteName>`). `xcodebuild` is unavailable.
- Toolchain trap 1 (resolved 2026-09-05, Task 1 re-verifies): two stale files, `arm64-apple-macos.private.swiftinterface` and its x86_64 twin (dated 2024-02-15, owned by no package) inside `/Library/Developer/CommandLineTools/usr/lib/swift/pm/ManifestAPI/PackageDescription.swiftmodule/`, shadowed the current public interface, so every `swift build` failed at the manifest link step with `Undefined symbols ... Package.__allocating_init`. They now live in `~/clt-stale-private-interfaces/`.
- Toolchain trap 2 (permanent, handled by the Makefile): the Command Line Tools' SwiftPM cannot wire in swift-testing on its own. It hands swiftc the framework directory with `-I` instead of `-F` and adds no rpath, so plain `swift test` fails with `no such module 'Testing'`. Manifest `swiftSettings` cannot fix it: SwiftPM's generated test runner is guarded by `#if canImport(Testing)` and compiles without manifest flags, so a manifest-only fix builds a runner with no entry point and `swift test` exits 0 having run zero tests. The `make test` target passes `-Xswiftc -F <CLT>/Library/Developer/Frameworks` plus `-Xlinker -rpath` for that directory and for `<CLT>/Library/Developer/usr/lib` (home of `lib_TestingInterop.dylib`). Verified 2026-09-05 on a spike package.
...
- Create: `Makefile` (test target only; Task 12 completes it)
...
```
Expected: exactly four entries, `arm64-apple-macos.swiftdoc`, `arm64-apple-macos.swiftinterface` and their x86_64 twins; no `*.private.swiftinterface`. If private interfaces are present, stop: Daniel moves them aside with `sudo mv .../PackageDescription.swiftmodule/*.private.swiftinterface ~/clt-stale-private-interfaces/` and the task restarts here. Do not attempt `SWIFTPM_CUSTOM_LIBS_DIR` as a workaround and do not put swift-testing search paths into `Package.swift` (see Global Constraints, trap 2).
...
`Makefile` (recipe lines are indented with a real tab character; Task 12 adds the remaining targets to this same file):
```make
...
Run: `make test FILTER=SnapshotCodingTests`
...
Run: `make test FILTER=StatuslinePayloadTests`
...
Run: `make test FILTER=SnapshotMergeTests`
...
Run: `make test FILTER=SnapshotStoreTests`
...
Run: `make test FILTER=StatuslineLineTests`
...
Run: `make test FILTER=HookRunnerTests`
...
Run: `make test FILTER=CountdownTests`
...
Run: `make test FILTER=DisplayStateTests`
...
Run: `make test FILTER=ClaudeGlyphTests`
...
- Modify: `Makefile` (created in Task 1 with the `test` target)
...
- [ ] **Step 2: Complete the Makefile**

Replace `Makefile` with the full version below. The `CLT_DEV`, `TEST_FLAGS`, `FILTER` block and the `test` recipe are unchanged from Task 1 (recipe lines are indented with a real tab character):
```make
# The Command Line Tools' SwiftPM does not wire in swift-testing on its own:
# it passes the framework directory with -I instead of -F and adds no rpath.
# When Xcode's toolchain is selected the directory is absent and no flags
...
### Task 14: Display refinements after first look (executes before Task 13)

Daniel's review of the installed app on 2026-09-05: keep the star always
filled and put a wider gap between it and the numbers; bold at 75 percent,
not 70; at 90 percent flip the whole label (white background at 50 percent
alpha, star and characters black); show each dropdown window as a white
pill bar. Pill bars need `.menuBarExtraStyle(.window)`, since a native menu
cannot host custom views.

**Files:**
- Modify: `Sources/MeterCore/DisplayState.swift`
- Modify: `Tests/MeterCoreTests/DisplayStateTests.swift`
- Modify: `Sources/MeterCore/ClaudeGlyph.swift` (drop the outline variant)
- Modify: `Tests/MeterCoreTests/ClaudeGlyphTests.swift`
- Create: `Sources/AIUsageMeter/LabelImage.swift` (replaces `GlyphImage.swift`)
- Delete: `Sources/AIUsageMeter/GlyphImage.swift`
- Modify: `Sources/AIUsageMeter/MeterModel.swift`
- Modify: `Sources/AIUsageMeter/MeterViews.swift`
- Modify: `Sources/AIUsageMeter/MeterApp.swift`
- Modify: `knowledge/decisions/2026-09-05_app-refresh-and-display-rules.md`
- Modify: `knowledge/decisions/2026-09-05_glyph-embedded-svg-string.md`

**Interfaces:**
- Consumes: `Snapshot`, `UsageWindow`, `Countdown` (unchanged), `ClaudeGlyph.filledSVG`.
- Produces: `WindowDisplay { percent: Int?, isBold: Bool, percentText: String, rowText: String, fraction: Double }`, `MeterDisplay { fiveHour, sevenDay, isFlipped: Bool, barText, ageText }`, `MeterDisplay.boldThreshold == 75`, `MeterDisplay.flipThreshold == 90`, `ClaudeGlyph.svg: String`. `glyphFilled`, `fillThreshold`, `ClaudeGlyph.outlineSVG` and `ClaudeGlyph.svg(filled:)` are removed.

- [ ] **Step 1: Update the display-state tests**

Replace `Tests/MeterCoreTests/DisplayStateTests.swift` with:
```swift
import Foundation
import Testing
@testable import MeterCore
...
Replace `Tests/MeterCoreTests/ClaudeGlyphTests.swift` with:
```swift
import Foundation
import Testing
@testable import MeterCore
...
Run: `make test FILTER=DisplayStateTests`
Expected: compile failure mentioning `isFlipped` / `fraction` (the members do not exist yet).

- [ ] **Step 3: Update DisplayState and ClaudeGlyph**

Replace `Sources/MeterCore/DisplayState.swift` with:
```swift
import Foundation
...
In `Sources/MeterCore/ClaudeGlyph.swift`: rename `filledSVG` to `svg`, delete `outlineSVG` and `svg(filled:)`, and update the doc comment to: `/// The Claude asterisk, embedded so the app needs no resource bundle. Byte-identical to assets/claude.svg (a test enforces it).` The string literal itself must not change.

- [ ] **Step 4: Run the MeterCore tests**

Run: `make test`
Expected: 52 tests pass across nine suites (DisplayStateTests now 7, ClaudeGlyphTests now 1; the app target does not compile yet, so if the build fails on `Sources/AIUsageMeter`, continue to Step 5 and run the suite again after it).

- [ ] **Step 5: Render the label as one image**

Delete `Sources/AIUsageMeter/GlyphImage.swift`. Create `Sources/AIUsageMeter/LabelImage.swift`:
```swift
import AppKit
import MeterCore
...
Replace `Sources/AIUsageMeter/MeterModel.swift` with:
```swift
import AppKit
import Foundation
import Observation
...
- [ ] **Step 6: Window-style dropdown with pill bars**

Replace `Sources/AIUsageMeter/MeterViews.swift` with:
```swift
import AppKit
import SwiftUI
import MeterCore
...
Replace `Sources/AIUsageMeter/MeterApp.swift` with:
```swift
import SwiftUI
...
- [ ] **Step 7: Build, run the suite, install, and look**
...
```
Expected: `Build complete`, 52 tests in nine suites pass, no warnings.

Then install the refreshed app (`make install` quits the running copy, replaces it in `~/Applications`, relaunches it, prints the snippet) and capture the menu bar:
```bash
make install 2>&1 | tail -3
sleep 3
screencapture -x -R 0,0,3000,30 /tmp/menubar-normal.png
```
Open `/tmp/menubar-normal.png` with the Read tool. Expected: a filled Claude asterisk, a visible gap, then `21% · 4%` (or whatever the snapshot holds) in the bar's own tint, with no background behind the label.

Now the flipped state, using a temporary snapshot:
```bash
S="$HOME/Library/Application Support/ai-usage-meter/snapshot.json"
cp "$S" /tmp/snapshot.backup.json
python3 - "$S" <<'EOF'
import json, sys
p = sys.argv[1]; d = json.load(open(p))
d["providers"]["claude"]["five_hour"]["used_percentage"] = 95
json.dump(d, open(p, "w"), indent=2)
EOF
osascript -e 'tell application "AI Usage Meter" to quit'; sleep 1
open "$HOME/Applications/AI Usage Meter.app"; sleep 3
screencapture -x -R 0,0,3000,30 /tmp/menubar-flipped.png
cp /tmp/snapshot.backup.json "$S"
osascript -e 'tell application "AI Usage Meter" to quit'; sleep 1
open "$HOME/Applications/AI Usage Meter.app"
```
Open `/tmp/menubar-flipped.png` with the Read tool. Expected: the label sits on a rounded, half-transparent white background with the star and `95% · 4%` in black, and `95%` bold. Confirm the snapshot file is restored (`grep used_percentage "$S"` shows the original values) and the app is running again.

The dropdown cannot be opened from a script without accessibility permission; state in the report that its visual check (two rows, each with a white pill bar, the age line, Quit) is left to the controller.

- [ ] **Step 8: Update the two decision pages**

In `knowledge/decisions/2026-09-05_app-refresh-and-display-rules.md`, replace the two bullets "Thresholds, monochrome only: ..." and "Glyph: ..." in "What was decided" with:
```markdown
...
```
Add under "## Why" a final paragraph:
```markdown
...
```
Add before the `**Last updated**` line:
```markdown
## History
...
In `knowledge/decisions/2026-09-05_glyph-embedded-svg-string.md`, change the Evidence bullet "Display decision: outline normally, filled at 90 percent, template image." to "Display decision: always filled, drawn into the label image (template normally, opaque when flipped)." and add before `**Last updated**`:
```markdown
## History
...
```bash
git add Sources Tests knowledge/decisions
git commit -m "feat: filled glyph, 75/90 flip rule, and pill-bar dropdown"
```
...
### Task 15: Effort level in the terminal line (requested 2026-09-05 after first use)

Daniel asked for the session's effort level as one word between the model
and the context segment: `Fable 5.1 · high · ctx 38% · 5h 2% · 7d 10%`.
The statusline payload carries it as `effort.level` (values seen: medium,
high, xhigh; probe file S2). When the payload has no effort block the
segment is omitted, not dashed: it is a session property, not a meter.

**Files:**
- Modify: `Sources/MeterCore/StatuslinePayload.swift`
- Modify: `Sources/MeterCore/StatuslineLine.swift`
- Modify: `Tests/MeterCoreTests/Fixtures.swift` (add `"effort":{"level":"high"}` to `samplePayloadJSON`)
- Modify: `Tests/MeterCoreTests/StatuslineLineTests.swift`
- Modify: `Tests/MeterCoreTests/StatuslinePayloadTests.swift` (one decode assertion)
- Modify: `Tests/MeterCoreTests/HookRunnerTests.swift` (two expected strings)
- Modify: `README.md` line 13, `knowledge/decisions/2026-09-05_hook-is-a-swift-target.md` line 8, `knowledge/entities/runbooks/install_and_wire.md` line 18 (the line format)

**Interfaces:**
- Produces: `StatuslinePayload.effort?.level: String?`; `StatuslineLine.render` output gains the optional effort segment.

- [ ] **Step 1: Tests first**

In `Tests/MeterCoreTests/Fixtures.swift`, add `"effort":{"level":"high"},` as a new line right after the `"version":"2.1.261",` line of `samplePayloadJSON`. Leave the other two fixtures without an effort block.

In `Tests/MeterCoreTests/StatuslineLineTests.swift`, change the first expectation to `"Fable 5.1 · high · ctx 12% · 5h 21% · 7d 4%"` and add:
```swift
...
```
(`rendersDashesForMissingWindows` already covers the absent case with the same string; keep it and let `omitsEffortWhenAbsent` name the intent.)

In `Tests/MeterCoreTests/StatuslinePayloadTests.swift`, inside the test that decodes `Fixtures.samplePayloadJSON` (`decodesTheRealCapture` or the equivalent), add `#expect(payload.effort?.level == "high")`.

In `Tests/MeterCoreTests/HookRunnerTests.swift`, change both `"Fable 5.1 · ctx 12% · 5h 21% · 7d 4%"` expectations to `"Fable 5.1 · high · ctx 12% · 5h 21% · 7d 4%"`.

- [ ] **Step 2: Run to see them fail**

Run: `make test FILTER=StatuslineLineTests`
Expected: compile failure, `value of type 'StatuslinePayload' has no member 'effort'`.

- [ ] **Step 3: Implement**

In `Sources/MeterCore/StatuslinePayload.swift`, add next to `Model`:
```swift
    public struct Effort: Decodable, Sendable {
        public var level: String?
    }
```
add the stored property `public var effort: Effort?` after `model`, and the coding key `case effort`.

In `Sources/MeterCore/StatuslineLine.swift`, replace `render` with:
```swift
    public static func render(_ payload: StatuslinePayload?) -> String {
        let model = payload?.model?.displayName ?? fallbackModel
...
Run: `make test`
Expected: 53 tests in nine suites pass (51 plus the two new StatuslineLine tests), no warnings.

- [ ] **Step 5: Docs, reinstall, commit**

Update the three documentation mentions of the line format: README line 13 becomes `Fable 5.1 · high · ctx 12% · 5h 21% · 7d 4%` and gains the clause "(the effort word is omitted when Claude Code does not send one)"; the hook decision page's format string becomes `<model> · <effort> · ctx <n>% · 5h <n>% · 7d <n>%` with a History bullet dated 2026-09-05 explaining the addition; the runbook check line becomes the same format. Keep `**Last updated**: 2026-09-05` on both wiki pages.

Run `make install 2>&1 | tail -3` so the hook in `~/.local/bin` carries the change (this also relaunches the app; confirm `pgrep -x AIUsageMeter`). Then `echo '{"model":{"display_name":"X"},"effort":{"level":"low"}}' | ~/.local/bin/ai-usage-meter-hook` must print `X · low · ctx -- · 5h -- · 7d --`.

```bash
git add Sources Tests README.md knowledge
git commit -m "feat: effort level in the terminal statusline line"
```

---

### Task 16: Context glyph replaces the ctx word (requested 2026-09-05 after Task 15)

Daniel asked for the stacked-cylinder symbol that Claude Code's `/context`
grid draws instead of the word `ctx`. He confirmed by eye that U+26C1
(`⛁`, WHITE DRAUGHTS KING, which Apple's fonts render as a database
cylinder) is the one; U+26C3 was the rejected alternative. It is a Unicode
symbol, not an emoji, so the no-emoji rule is not touched. Final line:
`Fable 5.1 · high · ⛁ 38%`.

**Files:**
- Modify: `Sources/MeterCore/StatuslineLine.swift`
- Modify: `Tests/MeterCoreTests/StatuslineLineTests.swift` (five expected strings)
- Modify: `Tests/MeterCoreTests/HookRunnerTests.swift` (five expected strings)
- Modify: `README.md` line 13, `knowledge/decisions/2026-09-05_hook-is-a-swift-target.md` line 8 plus a History bullet, `knowledge/entities/runbooks/install_and_wire.md` line 18

**Interfaces:**
- Produces: `StatuslineLine.contextGlyph: String` (`"\u{26C1}"`); `render` output replaces `ctx ` with the glyph and a space.

- [ ] **Step 1: Tests first**

In `Tests/MeterCoreTests/StatuslineLineTests.swift` and `Tests/MeterCoreTests/HookRunnerTests.swift`, replace every `ctx ` inside an expected string with `⛁ ` (the literal character U+26C1 followed by a space). The resulting strings are `"Fable 5.1 · high · ⛁ 12%"`, `"Fable 5.1 · ⛁ 3%"`, `"Opus 4.8 · medium · ⛁ --"`, `"Opus 4.8 · ⛁ 41%"`, `"Claude · ⛁ --"`. Add one test to `StatuslineLineTests`:
```swift
...
Run: `make test FILTER=StatuslineLineTests`
Expected: compile failure, `type 'StatuslineLine' has no member 'contextGlyph'`.

- [ ] **Step 3: Implement**

In `Sources/MeterCore/StatuslineLine.swift`, add after `fallbackModel`:
```swift
    /// U+26C1, the stacked-cylinder symbol Claude Code's /context grid uses.
    public static let contextGlyph = "\u{26C1}"
```
and change the `render` return to:
```swift
...
Run: `make test`
Expected: 54 tests in nine suites pass, no warnings.

- [ ] **Step 5: Docs, reinstall, commit**

README line 13: the example becomes `Fable 5.1 · high · ⛁ 12%` and the sentence gains "the cylinder is the same U+26C1 symbol /context uses for the context window". Hook decision page line 8: `<model> · <effort> · ⛁ <n>%`, plus a History bullet dated 2026-09-05: "ctx replaced by U+26C1, the /context cylinder, at Daniel's request; he confirmed the glyph by eye against U+26C3." Runbook line 18: same format. Keep `**Last updated**: 2026-09-05`.

Run `make install 2>&1 | tail -3` (relaunches the app; confirm `pgrep -x AIUsageMeter`). Then `echo '{"model":{"display_name":"X"},"effort":{"level":"low"}}' | ~/.local/bin/ai-usage-meter-hook` must print `X · low · ⛁ --`.

```bash
git add Sources Tests README.md knowledge
git commit -m "feat: context glyph replaces ctx in the terminal line"
```
