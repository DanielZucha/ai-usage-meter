...
- Blocking precondition: on this Mac the Command Line Tools 26.3 install has a `PackageDescription` interface that does not match its dylib, so every `swift build` fails at the manifest link step. Task 1 verifies the fix (Command Line Tools update to 26.6) before anything else.
...
```
Expected: branch `feature/v1-meter` (created from `devel` if it did not exist); a Swift Package Manager version line. If a later step fails with `Undefined symbols ... PackageDescription.Package.__allocating_init`, stop: the Command Line Tools update has not been applied. Daniel runs `sudo softwareupdate -i "Command Line Tools for Xcode 26.6-26.6"` (or System Settings, Software Update) and the task restarts here.
...
## Toolchain trap
Command Line Tools 26.3 shipped a `PackageDescription` interface older
than its dylib, so every `swift build` failed at the manifest link step
with `Undefined symbols ... Package.__allocating_init`. The fix is the
Command Line Tools 26.6 update from Software Update. Recorded 2026-09-05.
