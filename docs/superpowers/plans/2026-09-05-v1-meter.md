...
@Suite struct SnapshotCodingTests {
    ...
    @Test func usesSnakeCaseKeysAndISO8601Dates() throws {
        ...
        #expect(json.contains("\"used_percentage\" : 21"))
        #expect(json.contains("\"resets_at\" : \"2026-09-05T14:10:00Z\""))
        #expect(json.contains("\"captured_at\" : \"2026-09-05T11:48:12Z\""))
        #expect(json.contains("\"source\" : \"statusline\""))
    }
...
- Produces: `SnapshotStore(fileURL: URL)`, `SnapshotStore.defaultURL() -> URL`, `SnapshotStore.default`, `store.read() -> Snapshot?`, `store.write(_:) throws`.
...
/// a failure must cost nothing but a missing update.
...
Expected: all tests pass (49 across the nine suites).
...
Run: `sfltool dumpbtm 2>/dev/null | grep -A3 "ai-usage-meter" | head -8 || echo "sfltool needs sudo; check System Settings > General > Login Items instead"`
Expected: an entry for `com.danielzucha.ai-usage-meter`, or a visual check in Login Items showing "AI Usage Meter" enabled.
...
- notes: three-target SwiftPM package, 46 MeterCore tests, make install wiring; Command Line Tools 26.3 manifest-link trap recorded in the runbook
```
...
- Test count: 4 + 6 + 8 + 6 + 4 + 6 + 6 + 6 + 3 = 49 tests across nine suites.
